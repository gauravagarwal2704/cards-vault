...
internal class SSDOcr private constructor(interpreter: InterpreterWrapper) :
    ...
    companion object {
        ...
        fun cameraPreviewToInput(
            ...
            val croppedCard = cropCameraPreviewToViewFinder(
                cameraPreviewImage,
                previewBounds,
                cardFinder,
            )
        ...
        /**
         * Prepare a gallery image for the same SSD PAN model used by live camera frames.
         * Gallery photos do not have preview/view-finder coordinates, so use the largest
         * centered crop matching a payment card's trained aspect ratio.
         */
        fun galleryBitmapToInput(bitmap: Bitmap): Input {
            val targetAspect = Factory.TRAINED_IMAGE_SIZE.width.toFloat() /
                Factory.TRAINED_IMAGE_SIZE.height
            ...
            val cropped = Bitmap.createBitmap(bitmap, left, top, cropWidth, cropHeight).let {
                if (it === bitmap) {
