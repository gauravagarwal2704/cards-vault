package com.cardswallet.cards_vault.cardscan
...
class CardScanHandler(
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        ...
        if (pendingResult != null) {
            diagnostic("Native card scan rejected", null, "scan_in_progress")
            result.error("scan_in_progress", "A card scan is already running.", null)
            return
        }

        pendingResult = result
        diagnostic("Native card scan requested", "started", null)
        try {
            ...
            diagnostic("Native card scan failed", null, error.javaClass.simpleName)
            pendingResult = null
    ...
    fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?): Boolean {
        ...
        if (resultCode == Activity.RESULT_OK &&
            ...
            diagnostic("Native card scan completed", "success", null)
            result.success(
                mapOf(
                    "status" to "completed",
                    "pan" to pan,
            ...
            diagnostic(
                "Native card scan completed",
                "cancelled",
                data?.getStringExtra(CARD_SCAN_RESULT_REASON) ?: "closed",
            )
            result.success(
                mapOf(
                    "status" to "canceled",
    ...
    override fun close() {
        ...
        if (pendingResult != null) {
            diagnostic("Native card scan failed", null, "scan_closed")
        }
        pendingResult = null
    }
